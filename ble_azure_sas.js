...
function onStart(request, response) {
  ...
  response.send(200, 'Successully start sending message to cloud', function (err) {
    if (err) {
      console.log('[IoT hub Client] Failed sending a method response:\n' + err.message);
    }
  });
}
...
(function (connectionString) {
    ...
    try {
        ...
        console.log('Failed to load config.json: ' + err.message);
        return;
    }
    ...
    client.open((err) => {
        ...
        setInterval(() => {
            client.getTwin((err, twin) => {
                if (err) {
                    console.log("get twin message error");
                    return;
                }
         ...
         function Azure_Send(buff)
         {
           ...
           client.sendEvent(message, (err) => {
               ...
               if (err) {
                  ...
                  console.log('Failed to send message to Azure IoT Hub' + err.message);
               } else {
